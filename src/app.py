...
class SafetyChecker:
    ...
    SAFE_FOLDERS = [
        ...
        '/Users/daniel/X10/Main',  # X10 Main archive
    ]
...
class FileSizeAnalyzer:
    ...
    @classmethod
    def analyze_folder(cls, folder: str, max_results: int = 5000, exclude_cloud: bool = True, progress_callback=None, wait_for_all: bool = False) -> Dict:
        """
...
class FileGenius:
    ...
    def set_folder(self, folder):
        ...
        # Check if this is a cloud/remote storage location
        is_cloud = 'iCloud' in folder or 'Mobile Documents' in folder or 'X10' in folder

        # Show result - use yellow background for cloud/remote, green for normal folders
        result_bg = '#FFF3CD' if is_cloud else '#E8F5E9'
        ...
        # Show cloud/remote storage warning inline if applicable
        if is_cloud:
            warning_frame = tk.Frame(result, bg=result_bg, pady=10)
            warning_frame.pack(fill=tk.X, pady=8)

            tk.Label(warning_frame,
                    text=" External/Archive Storage Location",
                    font=('Helvetica', 14, 'bold'),
                    bg=result_bg, fg='#856404').pack(anchor=tk.W)

            tk.Label(warning_frame,
                    text=" Files in external or archive storage may be synced or stored remotely.\n"
                         "Deleting them may affect other devices or backups.",
                    font=('Helvetica', 12),
        ...
        tk.Label(result, text=f" {message}", font=('Helvetica', 13, 'bold'),
                fg='#2E7D32' if not is_cloud else '#856404', bg=result_bg).pack(anchor=tk.W, pady=8)
    ...
    def do_analyze(self):
        ...
        def analyze_thread():
            try:
                ...
                # Only exclude cloud storage if user DIDN'T explicitly select a cloud folder
                is_cloud_folder = 'iCloud' in self.current_folder or 'Mobile Documents' in self.current_folder
                results = FileSizeAnalyzer.analyze_folder(
                    self.current_folder,
                    exclude_cloud=not is_cloud_folder,  # If they selected cloud storage, don't exclude it
